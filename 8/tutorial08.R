...
# 1
logy <- log(y)
plot(date, logy, type = "l", xlab = "Time", ylab = "share prices")

# 2
# Generate log returns of y
r <- diff(log(y))
plot(date[-1], r, type = "l", xlab = "Time", ylab = "returns")
...
for (p in 0:3)
{
  for (q in 0:3)
  {
    ...
    ic_arma1[i,] <- c(p, q, ARMA_est[[i]]$aic, ARMA_est[[i]]$bic)
  }
}
...
# Select the first 36 in the intersection set.
